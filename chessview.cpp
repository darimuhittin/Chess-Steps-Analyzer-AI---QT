...
void ChessView::addHighlight(ChessView::Highlight *h1)
{
    m_highlights.append(h1);
    update();
}

void ChessView::removeHighlight(ChessView::Highlight *h1)
{
    m_highlights.removeOne(h1);
    update();
}

void ChessView::setValue(int value)
{
    m_value = value;
    update();
}

void ChessView::drawGameOver()
{
    QPainter ptr(this);
    ptr.drawText(QRectF(200,200,400,100),"Game Over");
}
...
void ChessView::drawValue(QPainter *painter)
{
    QPen pen(Qt::red,20);
    painter->setPen(pen);
    painter->drawText(10,10,QString::number(m_value));
}
